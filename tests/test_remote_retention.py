...
def make_task(tmp_path, sftp, *, retention_days=21, apply=False, logger=None, **kwargs):
...
class TestRetentionDecision:
    ...
    def test_apply_deletes_only_beyond_the_window(self, tmp_path, fake_sftp_factory, logger):
        files, mtimes = remote_tree({
            "download/WH1/IPC-1/a/D_old.csv": 22,
            "download/WH1/IPC-1/a/D_edge.csv": 20,
    ...
    def test_pattern_protects_everything_else(self, tmp_path, fake_sftp_factory, logger):
        ...
        files, mtimes = remote_tree({
            "download/WH1/IPC-1/a/D_log.csv": 22,
            "download/WH1/IPC-1/a/U_log.csv": 22,
            "download/WH1/IPC-1/a/readme.txt": 22,
            "download/WH1/IPC-1/a/settings.json": 22,
            "download/WH1/IPC-1/a/report.csv": 22,      # 是 .csv 但沒有 D_/U_ 前綴
        })
        sftp = fake_sftp_factory(files=files, mtimes=mtimes)
    ...
    def test_unknown_mtime_is_kept(self, tmp_path, fake_sftp_factory, logger):
        ...
        files, mtimes = remote_tree({"download/WH1/IPC-1/a/D_a.csv": 22})
        sftp = fake_sftp_factory(files=files, mtimes=mtimes)
...
class TestLocalMirrorCheck:
    def test_size_mismatch_keeps_the_remote_copy(self, tmp_path, fake_sftp_factory, logger):
        ...
        files, mtimes = remote_tree({"download/WH1/IPC-1/a/D_a.csv": 22}, size=100)
        sftp = fake_sftp_factory(files=files, mtimes=mtimes)
    ...
    def test_missing_local_copy_is_kept(self, tmp_path, fake_sftp_factory, logger):
        """遠端窗 21 天 < 本地窗 30 天,所以遠端檔到期時本地那份還有 9 天壽命、**必須**還在。

        不在就代表我們根本沒拿到 —— 這是遠端窗刻意短於本地窗換來的保證,
        也是這道檢查與 30/30 時的降級版最大的差別。
        """
        files, mtimes = remote_tree({"download/WH1/IPC-1/a/D_a.csv": 22})
        sftp = fake_sftp_factory(files=files, mtimes=mtimes)
    ...
    def test_allow_missing_local_is_the_explicit_escape_hatch(
        self, tmp_path, fake_sftp_factory, logger
    ):
        """停機超過 9 天會留下一批本地已經沒有的殘留；收掉它們是明確的人工決定。"""
        files, mtimes = remote_tree({"download/WH1/IPC-1/a/D_a.csv": 40})
        sftp = fake_sftp_factory(files=files, mtimes=mtimes)
        mirror(tmp_path, ["download/WH1/IPC-1/a/keep_me.csv"])
        task = make_task(tmp_path, sftp, apply=True, logger=logger, allow_missing_local=True)

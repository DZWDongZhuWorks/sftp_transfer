...
| `o` / `O` | 循環排序欄位（嚴重度／更新時間／裝置名稱）／切換升降冪 |
| `/` | 搜尋（`Esc` 清除） |
...
- 預設 `嚴重度↓`。在**分群模式**下這與加入排序功能前的順序完全相同（`build_tree` 已依
  `(-嚴重度, 元件)` 排過，穩定排序回傳同一份清單），所以沒按 `o` 之前畫面不會有任何變化。
  平坦模式則是全域重排，會把各群組的異常裝置集中到最上面。
- 排序是**穩定**的：同鍵值維持資料層給的 船/IPC/元件 次序。
- 未回報過（無時間戳）的裝置在「更新時間」視為最舊。
- 排序也套用在分群模式的裝置列，所以 `o`／`O` 兩邊都有用；但**群組本身**的順序仍由
  資料層決定（依群組最嚴重狀態），不受 `o` 影響。
